...
public class NPCHealth : MonoBehaviour
{
    ...
    {   // is there an HP bar attached
        ...
        {   // Resize and colour the bar based on current HP
            ...
            if (fl_HP > fl_max_HP / 2)
                tx_HP_bar.GetComponent<Renderer>().material.color = Color.green;

            if ((fl_HP > fl_max_HP / 4) && (fl_HP < fl_max_HP / 2))
                tx_HP_bar.GetComponent<Renderer>().material.color = Color.yellow;

            if (fl_HP < fl_max_HP / 4)
                tx_HP_bar.GetComponent<Renderer>().material.color = Color.red;
        }
    }//-----   
